...
function command_make {
    ...
    for f in $(find . -maxdepth 2 -name '*.map' ) ; do
        ...
        echo "- $map_name"
        $LQ_BSP_PATH $LQ_BSP_FLAGS $map_name.map | grep -E -i "WARNING 16"
        $LQ_VIS_PATH $LQ_VIS_FLAGS $map_name.map | grep -E -i "WARNING 16"
        $LQ_LIG_PATH $LQ_LIG_FLAGS $map_name.map | grep -E -i "WARNING 16"
